...
export function DataTableFooter({ itemLabel, total, page, totalPages, limit, onPageChange, onLimitChange, children }: {
  ...
  return <div className="card-footer d-flex flex-wrap justify-content-between align-items-center gap-2">
    ...
    {children}
    <nav className="ms-auto" aria-label={`${itemLabel} pagination`}><ul className="pagination pagination-sm mb-0"><li className={`page-item ${page === 1 ? 'disabled' : ''}`}><button type="button" className="page-link" disabled={page === 1} onClick={() => onPageChange(page - 1)}>Previous</button></li>{pages.map((item, index) => item === 'ellipsis' ? <li className="page-item disabled" key={`ellipsis-${index}`}><span className="page-link">…</span></li> : <li className={`page-item ${item === page ? 'active' : ''}`} key={item}><button type="button" className="page-link" onClick={() => onPageChange(item)}>{item}</button></li>)}<li className={`page-item ${page === totalPages ? 'disabled' : ''}`}><button type="button" className="page-link" disabled={page === totalPages} onClick={() => onPageChange(page + 1)}>Next</button></li></ul></nav>
  </div>;
}
